...
static int
window_tree_fuzzy_match(const char *haystack, const char *needle)
{
...
window_tree_search(__unused void *modedata, void *itemdata, const char *ss)
{
	...
	case WINDOW_TREE_SESSION:
		if (s == NULL)
			return (0);
		return (window_tree_fuzzy_match(s->name, ss));
	case WINDOW_TREE_WINDOW:
		if (s == NULL || wl == NULL)
			return (0);
		return (window_tree_fuzzy_match(wl->window->name, ss));
	case WINDOW_TREE_PANE:
		...
		if (cmd == NULL || *cmd == '\0')
			return (0);
		retval = window_tree_fuzzy_match(cmd, ss);
		free(cmd);
		return (retval);
	}
	return (0);
    ...
    struct args *args)
{
	...
	if (data->type == WINDOW_TREE_SESSION) {
		mode_tree_set_hint(data->data,
		    "Enter:switch  c:new  x:kill  X:kill-tagged"
		    "  t:tag  T:clear  ^T:tag-all  /:search  q:quit");
	} else {
		mode_tree_set_hint(data->data,
		    "Enter:select  x:kill  X:kill-tagged"
		    "  t:tag  T:clear  ^T:tag-all  /:search  q:quit");
	}
...
static int
window_tree_new_session_callback(struct client *c, void *modedata,
    const char *s, __unused int done)
{
	struct window_tree_modedata	*data = modedata;
	char				*cmd;
	...
	if (s != NULL && *s != '\0') {
		xasprintf(&cmd, "new-session -s '%s'", s);
		mode_tree_run_command(c, NULL, cmd, "");
		free(cmd);
		mode_tree_build(data->data);
		mode_tree_draw(data->data);
		data->wp->flags |= PANE_REDRAW;
...
again:
	...
	case 'c':
		data->references++;
		status_prompt_set(c, NULL, "New session name: ", "",
		    window_tree_new_session_callback, window_tree_command_free,
