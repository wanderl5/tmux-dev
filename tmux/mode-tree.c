...
struct mode_tree_data {
	...
	void			*searchdone_itemdata;
	struct client		*searchdone_client;
	int			 searchdone;
...
mode_tree_set_height(struct mode_tree_data *mtd)
{
	...
	if (screen_size_y(s) - mtd->height < 2)
		mtd->height = screen_size_y(s);
	if (mtd->hint_text != NULL && mtd->height > 0 &&
	    mtd->height < screen_size_y(s))
		mtd->height--;
}

void
mode_tree_set_hint(struct mode_tree_data *mtd, const char *text)
{
	mtd->hint_text = text;
}
...
mode_tree_draw(struct mode_tree_data *mtd)
{
	...
	/* Draw hint footer bar if set. */
	if (mtd->hint_text != NULL) {
		screen_write_cursormove(&ctx, 0, h, 0);
		screen_write_clearendofline(&ctx, gc.bg);
		screen_write_nputs(&ctx, w, &gc, " %s", mtd->hint_text);
	}
	...
	{
		u_int	hint_rows = (mtd->hint_text != NULL) ? 1 : 0;
		u_int	preview_y = h + hint_rows;
		u_int	preview_h = sy - h - hint_rows;

		screen_write_cursormove(&ctx, 0, preview_y, 0);
		screen_write_box(&ctx, w, preview_h, BOX_LINES_DEFAULT, NULL,
		    NULL);
		...
		if (w - 2 >= strlen(text)) {
			screen_write_cursormove(&ctx, 1, preview_y, 0);
		...
		if (box_x != 0 && box_y != 0) {
			screen_write_cursormove(&ctx, 2, preview_y + 1, 0);
			mtd->drawcb(mtd->modedata, mti->itemdata, &ctx,
			    box_x, box_y);
		}
	}
...
mode_tree_search_callback(struct client *c, void *data, const char *s, int done)
{
	...
	if (mode_tree_search_set(mtd) && done && mtd->searchdonecb != NULL) {
		mti = mtd->line_list[mtd->current].item;
		mtd->searchdone_itemdata = mti->itemdata;
		mtd->searchdone_client = c;
		mtd->searchdone = 1;
	}
...
mode_tree_search_free(void *data)
{
	...
	if (mtd->searchdone && !mtd->dead) {
		mtd->searchdonecb(mtd->modedata, mtd->searchdone_itemdata,
		    mtd->searchdone_client);
