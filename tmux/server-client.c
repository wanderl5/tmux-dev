...
static void	server_client_show_which_key(struct client *);
    ...
    overlay_resize_cb resizecb, void *data)
{
	...
	if (c->overlay_draw != NULL) {
		if (c->floating_popup != NULL &&
		    c->overlay_data == c->floating_popup) {
...
/*
 * Hide overlay on client without calling the free callback or destroying the
 * overlay data.  Used by toggle-floating-term to hide the popup while keeping
 * the underlying job and screen alive for later reattachment.
 */
void
server_client_hide_overlay(struct client *c)
{
	if (c->overlay_draw == NULL)
		return;
...
table_changed:
	...
	if ((key0 == (prefix & (KEYC_MASK_KEY|KEYC_MASK_MODIFIERS)) ||
		...
		server_client_show_which_key(c);
