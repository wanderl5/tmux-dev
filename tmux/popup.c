...
popup_draw_cb(struct client *c, void *data, struct screen_redraw_ctx *rctx)
{
	...
	if (pd->border_lines == BOX_LINES_NONE) {
		...
		screen_write_fast_copy(&ctx, &pd->s, 0,
		    pd->s.grid->hsize - pd->scroll_offset, pd->sx, pd->sy);
	} else if (pd->sx > 2 && pd->sy > 2) {
		screen_write_box(&ctx, pd->sx, pd->sy, pd->border_lines,
		    &pd->border_cell, pd->title);
		screen_write_cursormove(&ctx, 1, 1, 0);
		screen_write_fast_copy(&ctx, &pd->s, 0,
		    pd->s.grid->hsize - pd->scroll_offset, pd->sx - 2,
		    pd->sy - 2);
	}
...
popup_resize_cb(__unused struct client *c, void *data)
{
	...
	if (pd->scroll_offset > pd->s.grid->hsize)
		pd->scroll_offset = pd->s.grid->hsize;
}
...
popup_key_cb(struct client *c, void *data, struct key_event *event)
{
	...
	if (pd->job != NULL) {
		...
		if (pd->toggle_key != KEYC_NONE &&
		    (event->key & ~KEYC_IMPLIED_META) == pd->toggle_key) {
			server_client_hide_overlay(c);
			c->floating_popup_visible = 0;
			return (0);
		}
		if (event->key == KEYC_PPAGE) {
			u_int	inner_sy;

			inner_sy = (pd->border_lines == BOX_LINES_NONE)
			    ? pd->sy : pd->sy - 2;
			if (pd->scroll_offset + inner_sy <= pd->s.grid->hsize)
				pd->scroll_offset += inner_sy;
			else
				pd->scroll_offset = pd->s.grid->hsize;
			server_redraw_client(c);
			return (0);
		}
		if (event->key == KEYC_NPAGE) {
			u_int	inner_sy;

			inner_sy = (pd->border_lines == BOX_LINES_NONE)
			    ? pd->sy : pd->sy - 2;
			if (pd->scroll_offset >= inner_sy)
				pd->scroll_offset -= inner_sy;
			else
				pd->scroll_offset = 0;
			server_redraw_client(c);
    ...
    popup_close_cb cb, void *arg)
{
	...
	if (s != NULL)
		screen_init(&pd->s, jx, jy,
		    options_get_number(s->options, "history-limit"));
	else
		screen_init(&pd->s, jx, jy,
		    options_get_number(c->session->options, "history-limit"));
...
void
popup_reattach(struct popup_data *pd, struct client *c)
{
	server_client_unref(pd->c);
...
void
popup_set_toggle_key(struct popup_data *pd, key_code key)
{
	pd->toggle_key = key;
}
 ...
 */
void
popup_close_floating(struct client *c)
{
